...
//! which is fine for some use cases, but most users will want access to the other features.
//! 
...
//! ## Design
//! 
//! At the highest level, this crate is split into a low-level and raw kernel API ([`kapi`]) that can be used
//! directly for advanced use cases, and a higher-level interface which provides abstractions over this API
//! as well as some limited separate functionality that doesn't require the kernel module to be loaded
//! ([`RamInspector`]).
//! 
//! 
#![forbid(missing_docs)]
